...
gradients:
  brand-red-gradient:
    ...
    value: "linear-gradient(148.66deg, #F96766 0%, #E21413 100%)"
  action-gradient:
    angle: "148.66deg"
    start: "#F96766"
    end: "#E21413"
...
components:
  ...
  input-default:
    ...
    hoverBorder: "1px solid {colors.border-hover}"
...
`brand-red-gradient` 是 Foundations/Color 中的主按钮渐变定义：起始色 `#F96766`、结束色 `#E21413`、角度 `148.66deg`。`action-gradient` 是该既有渐变在组件中的引用 token；不得新建其他红色渐变。
...
状态颜色必须通过以下 token 引用，禁止 AI 自定义状态颜色。

| 状态 | Text Token / Value | Background Token / Value |
| --- | --- | --- |
| success | `success` / `#16A34A` | `success-subtle` / `#F0FDF4` |
| warning | `warning` / `#ED6C02` | `warning-subtle` / `#FFF4E5` |
| processing | `processing` / `#4590E6` | `processing-subtle` / `#EAF3FF` |
| error | `error` / `#E21413` | `error-subtle` / `#FFF1F1` |
| neutral | `neutral` / `#666666` | `neutral-subtle` / `#F5F5F5` |
...
- 菜单 hover 使用 `sidebar-hover`（`#F8F9FA`）；当前 active 使用 `brand-red-subtle`（`#FFF5F5`）、`brand-red` 文字，并带有 `brand-red` active indicator。
- hover 与 active 必须有清晰区别；禁止使用纯红大面积背景。

#### Content Workspace

- Content Workspace 使用 `workspace-canvas`（`#F5F7FA`）背景；主内容区域保持 fluid，不设置全局最大宽度。
...
- Page Container 使用 `surface`（`#FFFFFF`）背景、`rounded.md`（8px）圆角和 `spacing.lg`（24px）内边距。
...
- 主按钮使用 `button-primary`，背景固定为 `linear-gradient(148.66deg, #F96766 0%, #E21413 100%)`，文字为白色，承担高视觉权重的主要行动。
...
- Neutral Text Button Rules：`button-text` 是灰色纯文字按钮，使用 `text-secondary`；不允许 Icon、border 或 background。仅用于取消、返回、重置、取消选择、关闭等辅助操作。
...
- default 状态 value 使用 `text-primary`，placeholder 使用弱文本颜色 `text-muted`；hover 使用 `1px solid border-hover`。
...
- disabled 使用 `disabled-bg` 背景与 `disabled-text` 文字。
...
- 选中 Checkbox 使用 `brand-red`；选中行使用 `brand-red-subtle`（`#FFF5F5`）背景，且该背景仅作用于被选中的表格行。
